...
class FastAPISettings(BaseSettings):
    ...
    def parse_cors_origins(cls, v):
        ...
        if v is None or v == "":
            return ["http://localhost:3000", "http://localhost:3001", "http://127.0.0.1:3000", "https://email-parser-frontend-lyart.vercel.app/", "https://*.vercel.app"]
        if isinstance(v, str):
            # Handle comma-separated string from environment variable
            origins = [origin.strip() for origin in v.split(',') if origin.strip()]
            return origins if origins else ["http://localhost:3000", "http://localhost:3001", "http://127.0.0.1:3000", "https://email-parser-frontend-lyart.vercel.app",  "https://*.vercel.app"]
        return v
